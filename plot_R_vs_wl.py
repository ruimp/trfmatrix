...
d = 10
wl_list = np.linspace(500, 1600, 200)

T = trf.TransferMatrix(300, -0.2, wl_list)
T.get_graphene_xi()

fig, axs = plt.subplots(3, 1, sharex=True, sharey=True, gridspec_kw={'hspace':0.16, 'wspace':0.03})
...
for i, n_in in enumerate(n_in_list):
    ...
    for n_out in n_out_list:
        ...
        for j in range(T.n_wls):
            ...
            T.get_R()
            R[j] = T.R*100
#        axs[i//2, i%2].plot(wl_list, R, label=r"$n_f$={}".format(n_out))
#        axs[i//2, i%2].legend()
#        axs[i//2, i%2].set_title(r"{} layers".format(n_layers))
